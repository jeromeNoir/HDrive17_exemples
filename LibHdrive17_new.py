...
# MOVE TO POSITION:
# -mydrive.newPosition=90 #set the target position in deg, the drive will move to this position when you call mydrive.start()
# -mydrive.mode='move'
# -mydrive.start() #start moving to the target position, it will run in an independent thread, the thread will stop when the target position is reached, you can also stop it before
...
class hdrive:
    ...
    def start(self):
        ...
        if self.mode=='osc':
            ...
            print('start moving to position ')
            self.run = threading.Thread(target=self.move)
            self.run.start()
    ...
    def move(self):
        ...
        new_pos=self.newPosition
        new_pos*=self.gearRatio
        new_pos*=10

        t0,position,speed,torque=self.get_drive_output() #get the current position to compare with the target position
        print('Current position',position/10)
        print('Moving to position',new_pos/10)

        while np.abs(position-new_pos) > 0.1:  # Use np.abs for absolute difference
            ...
            self.act_time.append(t)
            self.act_pos.append(position/10)
            self.act_speed.append(speed)
